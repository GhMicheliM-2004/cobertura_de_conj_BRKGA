...
print(os.listdir("/content/drive/My Drive/config"))

def carregar_instancia_py(py_path):
    spec = importlib.util.spec_from_file_location("/content/drive/My Drive/arquivos_intancias", py_path)
...
def resolver_instancia(m, n, d, A, seed=42, config_file="/content/drive/My Drive/config/config.conf", num_generations=100):
    inicio = time.time()
...
# loop de instancias
arquivos_py = [f for f in os.listdir("/content/drive/My Drive/arquivos_instancias") if f.endswith(".py")]

for arquivo in arquivos_py:
    caminho = os.path.join("/content/drive/My Drive/arquivos_instancias", arquivo)
